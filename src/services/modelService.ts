...
/**
 * Uploads a model image file to the server
 * @param file The file to upload
 * @returns Promise with the upload response containing url
 */
export async function uploadModelImage(file: File): Promise<UploadResponse> {
  try {
...
/**
 * Deletes a model image from storage
 * @param url URL of the image to delete
 */
export async function deleteModelImage(url: string): Promise<void> {
  try {
    if (!url) return;
